...
- `datasets/prediction_datasets` Contains all data required for training IDDGCN, as well as pre-trained weights available in the subdirectory 'weights', which can be directly used for model evaluation.
...
```
3. **Ground Truth Construction**: You can use `ground_truth.py` in the `explanation` folder to construct ground truth, or directly use the pre-constructed groundtruth `gt_all.csv` in the `explanation/data` folder.
```markdown
cd explanation
