...
cexplainer_preds = np.load(f'../datasets/explanation_datasets/IDDGCN_explain_preds_fold{fold}.npz', allow_pickle=True)['preds']
...
IDDGCN_metrics = compute_metrics_for_method(cexplainer_preds, 'IDDGCN')
...
IDDGCN_metrics_df = pd.DataFrame(IDDGCN_metrics, columns=['Precision@5', 'Recall@5', 'F1@5'])
# Calculate averages
...
avr_IDDGCN_prec = IDDGCN_metrics_df['Precision@5'].mean()
avr_IDDGCN_rec = IDDGCN_metrics_df['Recall@5'].mean()
avr_IDDGCN_f1 = IDDGCN_metrics_df['F1@5'].mean()
...
print(f'Total average IDDGCN Precision@5: {avr_IDDGCN_prec:.4f}')
print(f'Total average IDDGCN Recall@5: {avr_IDDGCN_rec:.4f}')
print(f'Total average IDDGCN F1@5: {avr_IDDGCN_f1:.4f}')
