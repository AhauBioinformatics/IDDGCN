...
class IDDGCN_Model(tf.keras.Model):
    ...
    # 这是一个自定义的TensorFlow模型，重写了train_step方法，用于训练关系图卷积网络）
    ...
    def train_step(self, data):
        ...
        y_pos_true = data[1]
        # 生成负样本的头部和尾部
        # neg_head, neg_tail = utils1.get_negative_triples1(
        #     head=pos_head,
        #     rel=rel,
        #     tail=pos_tail,
        ...
        #     seed=self.seed
        # )
        # 去按照输入的正样本索引读已经准备好的负样本
        # 使用 pandas 读取 CSV 文件
        X_train_neg = np.load(f'../data/data_new_KRAS/mode{self.mode}_fold{self.fold}_X_train_neg.npy')
        # 获取数据集的长度（样本数）
        ...
        # tf.print(neg_head.shape)

        # tf.print(neg_head.shape)
        # tf.print(neg_head,summarize=50)
        # tf.print(neg_tail,summarize=50)
        ...
        with tf.GradientTape() as tape:
            ...
            # # 对梯度进行梯度裁剪
            # clip_norm = 0.05
            # clipped_gradients, _ = tf.clip_by_global_norm(grads, clip_norm)
            #
            # # 用梯度更新模型参数
            # self.optimizer.apply_gradients(zip(clipped_gradients, self.trainable_weights))
            #
            # # 更新模型的度量状态
            # self.compiled_metrics.update_state(y_pos_true, y_pos_pred)
            # 返回模型的度量
    ...
    for bs in batch_size:
        for lr in learning_rate:
            for ed in embedding_dim:
                ...
                test_set=pd.read_csv("../data/data_new_KRAS/test.csv",header=0)

                train_set = pd.read_csv("../data/data_new_KRAS/train.csv", header=0)

                cell_similar_triples = pd.read_csv(f"../data/data_new_KRAS/mu_similar_noKRAS.csv", header=None)

                drug_similar_triples = pd.read_csv(f"../data/data_new_KRAS/drug_similar0.78.csv", header=0)

                # X_train_neg = pd.read_csv('../../data/negative_dc_28_1754.csv', header=0)
                # ----------------------------------------------------保存三元组，以实体形式
                # 对resopnse_pairs设置5折交叉验证
                test_set.columns = ['obj', 'rel', 'sbj']
                train_set.columns = ['obj', 'rel', 'sbj']
                ...
                for mode in range(0,1):
                    ...
                    for fold in range(4,5):
                        train_set = pd.concat([train_set, cell_similar_triples,drug_similar_triples], axis=0)
                        X_train_response =train_set
                        X_test_response = test_set
                        # neg_X_train, neg_X_test = neg_train_test_splits[fold]
                        neg_X_train=pd.read_csv(f"../data/data_new_KRAS/dc_negative_noKRAS.csv")
                        neg_X_test=pd.read_csv(f"../data/data_new_KRAS/dc_negative_KRAS.csv")
                        neg_X_test_filtered = neg_X_test[neg_X_test['rel'].isin([0, 1])]
                        neg_X_test_filtered.to_csv(f'../data/data_new_KRAS/mode{mode}_fold{fold}_neg_X_test.csv',index_label=None)
                        # 获取X_train和X_test
                        ...
                        X_train.to_csv(f"../data/data_new_KRAS/mode{mode}_fold{fold}_X_train.csv", index=False)
                        X_test.to_csv(f"../data/data_new_KRAS/mode{mode}_fold{fold}_X_test.csv", index=False)

                        # ------------------------------------------------------提取node_representation
                        all_feature_matrix = pd.read_csv(f"../data/data_new_KRAS/feature_all_248.csv", header=None,index_col=0)
                        ...
                        np.save(f'../data/data_new_KRAS/mode{mode}_fold{fold}_X_train_neg.npy', X_train_neg)
                        ...
                        # 定义保存权重的文件名模板
                        save_path_template = os.path.join('..','data','data_new_KRAS', 'weights', 'KRAS','mode{mode}_fold{fold}_epoch{epoch}_learnRate{learning_rate}_batchsize{batch_size}_embdim{EMBEDDING_DIM}_weight.h5')
                        # 创建自定义回调函数
                        ...
                        np.save(f'../data/data_new_KRAS/mode{mode}_fold{fold}_learning_rate0.001_final_embeddings.npy', entity_embeddings)
