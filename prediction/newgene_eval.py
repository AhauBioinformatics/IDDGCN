...
import IDDGCN,IDDGCN_newgene,IDDGCN_newgene
...
mode = 0
fold = 4
X_train = pd.read_csv(f"../data/data_new_KRAS/mode{mode}_fold{fold}_X_train.csv")
...
# trained_embeddings = np.load(f'../../data_new_KRAS/mode{mode}_fold{fold}_learning_rate0.001_final_embeddings.npy')
x_all = pd.read_csv(f"../data/data_new_KRAS/feature_all_248.csv", header=None,index_col=0).values
...
model.load_weights(os.path.join( '..', 'data','data_new_KRAS', 'weights', 'KRAS',
                                f'mode{mode}_fold{fold}_epoch200_learnRate0.001_batchsize100_embdim{EMBEDDING_DIM}_weight.h5'))
